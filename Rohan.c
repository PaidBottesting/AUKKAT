...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        int targetPort = baseTargetPortत्न

        if (elapsed > 240) {
            targetPort += (rand() % 100); // Wider shift (0-99) after 240s
        }
        serverAddr.sin_port = htons(targetPort);

        // Burst of 5 packets
        for (int burst = 0; burst < 5 && keepSending; burst++) {
