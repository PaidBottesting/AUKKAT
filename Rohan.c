...
std::atomic<long long> totalPacketsSent(0); // Atomic for lock-free updates
long long totalSendFailures = 0;
double totalDataMB = 0.0;
std::mutex statsMutex; // Only for failures/data
...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        if (elapsed > 240 && elapsed % 60 == 0) { // Shift every 60s after 240s
            targetPort += (rand() % 10);
        }
        ...
        totalPacketsSent += successfulSends; // Atomic update
        if (threadId == 0 && elapsed % 30 == 0 && elapsed > 240) { // More frequent past 240s
