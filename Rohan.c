...
void packetSender(int threadId, const std::string& targetIp, int targetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        // Periodic rate check (every 60s)
        auto checkElapsed = std::chrono::duration_cast<std::chrono::seconds>(now - lastCheck).count();
        if (checkElapsed >= 60) {
            long long packetsThisPeriod = threadPackets - lastPackets;
            double rate = packetsThisPeriod / (double)checkElapsed;
            if (threadId == 0) { // Only thread 0 logs to avoid spam
                std::cout << "Thread 0 rate at " << elapsed << "s: " << rate << " packets/s\n";
            }
            lastPackets = threadPackets;
            lastCheck = now;
        }
    }
