...
void logRate() {
    ...
    while (keepSending) {
        ...
        if (elapsed > 0) {
            ...
            double totalRate = totalPacketsSent / elapsed;
            std::cout << "\nTotal rate at " << elapsed << "s: " << totalRate << " packets/s" << std::flush;
        }
    }
}
...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        int burstSize = 5;
        if (elapsed > 300) {
            targetPort += (rand() % 500); // Wider shift (0-499) after 300s
            burstSize = 10; // Bigger burst after 300s
        } else if (elapsed > 240) {
            targetPort += (rand() % 100); // 0-99 shift after 240s
        }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Starting rate logger...\n";
    std::thread rateThread(logRate);
