...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        int burstSize = 5; // Default 5 packets
        if (elapsed > 240) {
            targetPort += (rand() % 10); // Small shift (0-9) after 240s
            burstSize = 7; // Increase to 7 after 240s
        }
        ...
        // Log burst success (thread 0 only, every 60s)
        if (threadId == 0 && elapsed % 60 == 0 && elapsed > 0) {
