...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    // Random source port (simplified to 0 for OS assignment, avoiding bind issues)
    struct sockaddr_in localAddr;
    localAddr.sin_family = AF_INET;
    localAddr.sin_addr.s_addr = INADDR_ANY;
    localAddr.sin_port = 0; // Let OS pick, avoids port conflicts
    ...
    while (keepSending) {
        ...
        // Fixed packet allocation outside fragment loop
        char* packet = new char[packetSize];
        if (!packet) {
            std::cerr << "Thread " << threadId << ": Memory allocation failed\n";
            close(udpSocket);
            return;
        }

        // BGMI-like payload (single packet, no fragments yet)
