...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        // Slight port variation after 240s
        int targetPort = baseTargetPort;
        if (elapsed > 240) {
            targetPort += (rand() % 10); // Small shift (0-9) to keep pressure
        }
        ...
        // Fixed rate check (total rate, thread 0 only)
        auto checkElapsed = std::chrono::duration_cast<std::chrono::seconds>(now - lastCheck).count();
        if (checkElapsed >= 60 && threadId == 0) {
            std::lock_guard<std::mutex> lock(statsMutex);
            double totalRate = (totalPacketsSent - totalPacketsReceived) / (double)checkElapsed;
            std::cout << "Total rate at " << elapsed << "s: " << totalRate << " packets/s\n";
            totalPacketsReceived = totalPacketsSent; // Reset for next period
            lastCheck = now;
        }
    }
