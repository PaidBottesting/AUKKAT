...
# Keep all load/save functions unchanged
def load_users():
    try:
...
def handle_broadcast(message):
    ...
    for group_id in current_group_ids:
        retries = 2  # Number of retries for each group
        success = False
        for attempt in range(retries):
            try:
                ...
                print(f"Attempt {attempt + 1} failed for {group_id}: {e}")
                if attempt == retries - 1:  # Last attempt
                    failed_groups[group_id] = str(e)
                time.sleep(2 ** attempt)  # Exponential backoff
    
        # If all retries failed, remove invalid group ID
        if not success:
            if group_id in group_ids:
                group_ids.remove(group_id)
                print(f"Removed invalid group ID: {group_id}")
...
# Keep other command handlers from previous fix
...
def handle_shutdown(message):
    ...
    for group_id, process in list(active_attacks.items()):
        try:
            ...
            print(f"Failed to stop attack in {group_id}: {e}")
