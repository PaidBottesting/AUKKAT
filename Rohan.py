...
# Bot configuration
BOT_USERNAME = "@PUBLIC_DDOSBOT"  # Add your bot's username here
...
# Updated command handlers to handle bot username
...
def handle_shutdown(message):
    ...
    # Immediate termination of all attacks
    for group_id, process in list(active_attacks.items()):
        try:
            process.kill()  # Use kill first for immediate termination
            time.sleep(0.5)
            if process.poll() is None:  # Double-check
                process.terminate()
    ...
    save_users()
    save_groups()
    bot.stop_bot()  # More reliable than stop_polling
    logger.info(f"Bot shutdown by {user_id}")
    raise SystemExit

# Keep other handlers unchanged (for brevity, assuming they work as expected)
...
def handle_stop(message):
    ...
    attack_process.kill()  # Immediate kill
    time.sleep(0.5)
